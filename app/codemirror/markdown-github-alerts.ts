  ...
  md.core.ruler.after("block", "github_alerts", (state) => {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        for (let j = i + 1; j < closeIndex; j++) {
            ...
            if (tokens[j].type === "blockquote_open" || tokens[j].type === "list_item_open") {
                break;
            }
        }
        ...
        const markerRegex = /(?:^|\n)\s*\[!(\w+)\]\s?(.*)/g;
        const matches = Array.from(firstInline.content.matchAll(markerRegex));
        
        if (matches.length === 0 || matches[0].index !== 0) continue;

        
        const match0 = matches[0];
        if (!match0) continue;
        
        const type0 = match0[1].toLowerCase();
        const rawTitle0 = match0[2];
        const title0 = rawTitle0 || type0.charAt(0).toUpperCase() + type0.slice(1);

        
        openToken.tag = "div";
        openToken.attrs = [["class", `callout callout-${type0}`]];
        
        const headerTokens0 = createHeaderTokens(state, type0, title0);
        const divContentOpen0 = new state.Token('div_open', 'div', 1);
        divContentOpen0.attrs = [['class', 'callout-content']];
        
        
        if (!match0[0]) continue;
        const cutLength = match0[0].length;
        ...
        if (newlineMatch) {
            firstInline.content = firstInline.content.substring(newlineMatch[0].length);
            strippedNewline = true;
        }
        
        tokens.splice(i + 1, 0, ...headerTokens0, divContentOpen0);
        
        let insertedCount = headerTokens0.length + 1;
        ...
        let ejectionLine = -1;
        if (openToken && openToken.map && state.src) {
        ...
        while (cursor < currentCloseIndex) {
            ...
            if (token.type === 'paragraph_open' && ejectionLine !== -1 && token.map) {
                const [start, _] = token.map;
            ...
            if (token.type === 'inline') {
                ...
                if (ejectionLine !== -1) {
                    ...
                    if (doubleNewlineIndex !== -1) {
                        ...
                        let pStartLine = -1;
                        if (tokens[cursor - 1] && tokens[cursor - 1].type === 'paragraph_open' && tokens[cursor - 1].map) {
                             pStartLine = tokens[cursor - 1].map![0];
                        }
                        
                        if (pStartLine !== -1 && strippedNewline && cursor === firstPIndex + 1) {
                            pStartLine += 1;
                        }
                ...
                const innerMatches = Array.from(content.matchAll(/(?:^|\n)\s*\[!(\w+)\]\s?(.*)/g));
                ...
                if (innerMatches.length > 0) {
                     ...
                     if (splitIndex === -1 || m.index < splitIndex) {
                         ...
                         newCalloutTitle = m[2] || newCalloutType.charAt(0).toUpperCase() + newCalloutType.slice(1);
                     }
                }
                ...
                if (splitIndex !== -1) {
                     ...
                     if (splitType === 'match') {
                        startPost += innerMatches[0][0].length; 
                     }
                     ...
                     let tokensToInsert: any[] = [];
                     
                     if (splitType === 'match') {
                         const divCalloutOpen = new state.Token('div_open', 'div', 1);
                         divCalloutOpen.attrs = [['class', `callout callout-${newCalloutType}`]];
                         const headerTokens = createHeaderTokens(state, newCalloutType, newCalloutTitle);
                         const divContentOpen = new state.Token('div_open', 'div', 1);
                         divContentOpen.attrs = [['class', 'callout-content']];
                         
                         tokensToInsert = [
                             pCloseNew,
                             divContentClose, divCalloutClose,
                             divCalloutOpen, ...headerTokens, divContentOpen,
                             pOpenNew, newInline
                         ];
                     } else {
                          ...
                          const alertInPostMatch = postText.match(/^([\s\S]*?)(?:^|\n)\s*\[!(\w+)\]\s?(.*)/m);
                          
                          if (alertInPostMatch && alertInPostMatch[2]) {
                              
                              const plainText = alertInPostMatch[1] || '';
                              const newAlertType = alertInPostMatch[2].toLowerCase();
                              const newAlertTitle = alertInPostMatch[3] || newAlertType.charAt(0).toUpperCase() + newAlertType.slice(1);
                              
                              
                              const afterMarkerIndex = (alertInPostMatch.index || 0) + alertInPostMatch[0].length;
                              ...
                              const divCalloutOpen = new state.Token('div_open', 'div', 1);
                              divCalloutOpen.attrs = [['class', `callout callout-${newAlertType}`]];
                              const headerTokens = createHeaderTokens(state, newAlertType, newAlertTitle);
                              const divContentOpen = new state.Token('div_open', 'div', 1);
                              divContentOpen.attrs = [['class', 'callout-content']];
                              ...
                              alertInline.content = alertContent.replace(/^\s*>\s?/gm, '').trim();
                              ...
                              if (plainText.trim()) {
                                  tokensToInsert = [
                                      pCloseNew,
                                      divContentClose, divCalloutClose,
                                      pOpenNew, plainInline, new state.Token('paragraph_close', 'p', -1),
                                      divCalloutOpen, ...headerTokens, divContentOpen,
                                      pOpen2, alertInline, pClose2,
                                      divContentClose2, divCalloutClose2
                                  ];
                              } else {
                                  tokensToInsert = [
                                      pCloseNew,
        ...
        if (cursor >= currentCloseIndex && tokens[currentCloseIndex] && tokens[currentCloseIndex].type === 'blockquote_close') {
             const closeToken = tokens[currentCloseIndex];
             closeToken.tag = "div";
...
function createHeaderTokens(state: any, type: string, title: string): any[] {
      ...
      const iconHTML = iconMap[type] || iconMap['note'] || "";
...
function findBlockQuoteClose(tokens: any[], startIndex: number): number {
    let level = 0;
