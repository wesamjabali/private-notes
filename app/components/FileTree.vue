...
const CONTEXT_MENU_KEY = Symbol.for('fileTreeContextMenu')
...
const handleContextMenu = (e: MouseEvent, node: FileNode) => {
    ...
    if (node.type === 'tree') {
        contextMenu.value.actions.unshift(
            { label: 'Set as Root', icon: '📌', action: () => store.setMainFolder(node.path) },
            { label: 'New Note', icon: '📝', action: () => {
            ...
            { label: 'Upload Files', icon: '📤', action: () => triggerUpload(node.path) }
        )
    }
}
...
const handleRootContextMenu = (e: MouseEvent) => {
    ...
    contextMenu.value = {
        ...
        y: e.clientY,
        actions: [
            { label: 'New Note', icon: '📝', action: () => store.startCreation(null, 'blob') },
            { label: 'New Folder', icon: '📁', action: () => store.startCreation(null, 'tree') },
...
const confirmDelete = async (node: FileNode) => {
    if (confirm(`Are you sure you want to delete ${node.name}?`)) {
        try {
            await store.deleteFile(node.path, node.sha || 'tree', `Delete ${node.name}`)
        } catch (e: any) {
             alert(e.message)
        }
    }
...
watch(() => store.currentFilePath, async (path) => {
  if (!path) return
...
const onNodeDrop = async (e: DragEvent, node: FileNode) => {
    ...
    if (rawData) {
        ...
        } catch (e) { console.error('Invalid drop data', e) }
        return
    }
...
<template>
  <div 
    class="file-tree" 
    :class="{ 'root-drag-over': depth === undefined && dragOverPath === 'root' }"
  ...
  >
    <div v-for="node in nodes" :key="node.path">
      ...
      <!-- Recursive Children -->
      <div v-if="node.type === 'tree' && expandedPaths.has(node.path)" class="children">
        <!-- Creation Input (if this node is the parent) -->
        <div 
             v-if="store.pendingCreation && store.pendingCreation.parentPath === node.path" 
             class="node-item creation-item"
        >
             <span class="chevron-placeholder"></span>
             <div class="node-content">
                  <span class="icon">{{ store.pendingCreation.type === 'tree' ? '📁' : '📝' }}</span>
                  <input 
                      ref="creationInput"
        ...
        <FileTree v-if="node.children && node.children.length > 0" :nodes="node.children" :depth="(depth || 0) + 1" />
      </div>
    </div>

    <!-- Root Level Creation Input -->
    <div 
         v-if="store.pendingCreation && store.pendingCreation.parentPath === null && depth === undefined" 
         class="node-item creation-item"
    >
         <div class="node-content">
              <span class="icon">{{ store.pendingCreation.type === 'tree' ? '📁' : '📝' }}</span>
